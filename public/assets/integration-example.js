...
class WorkflowApp {
  ...
  start() {
    ...
    this.mouse = new Mouse(this.options.canvas, this.renderManager, this.scene, this.select, this.keyControl, this.eventManager);
    this.gridDraw = new GridDraw(() => this.renderManager?.getViewportSize() ?? { width: 0, height: 0 }, () => this.renderManager?.getViewTransform() ?? { x: 0, y: 0, scale: 1 });
  ...
  resize() {
    ...
    const height = Math.max(1, Math.round(rect.height || this.options.canvas.clientHeight || this.options.canvas.height || 1));
...
function clonePortModel(port) {
  return { ...port };
}
function cloneEdgeModel(edge) {
  return { ...edge };
}
// src/integration-example.ts
var canvas = document.querySelector("#app");
var status = document.querySelector("#integration-status");
var logElement = document.querySelector("#integration-log");
var startButton = document.querySelector("#start-app");
var destroyButton = document.querySelector("#destroy-app");
var focusButton = document.querySelector("#focus-review");
var toggleStatusButton = document.querySelector("#toggle-review-status");
var updateNodeButton = document.querySelector("#update-start-node");
var togglePortButton = document.querySelector("#toggle-tone-output");
var createNodeButton = document.querySelector("#create-insight-node");
...
function updatePromptNode() {
  ...
  mountApp().updateNode("prompt", {
    ...
    parameters: current.parameters.map((row) => row.label === "Model" ? { ...row, value: nextVersion.endsWith("v2") ? "gpt-5.4" : "gpt-5.4-mini" } : row)
  });
...
function toggleToneOutput() {
  ...
  const nextOutputs = hasTonePort ? current.outputs.filter((port) => port.id !== "tonePreset") : [...current.outputs, createTonePort()];
  const nextParameters = current.parameters.map((row) => row.label === "Output Schema" ? {
    ...row,
