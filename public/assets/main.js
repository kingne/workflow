...
var scene = new Scene(nodes, edges, history);
renderManager.setAnimationResolver(() => scene.hasActiveAnimations());
var select = new Select(scene, renderManager);
...
var titleTextDraw = new TextDraw({
...
var helperTextDraw = new TextDraw({
...
var overlayDraw = new GroupDraw([titleTextDraw, helperTextDraw], "screen");
scene.getNodeDraws().forEach((nodeDraw) => {
  nodeDraw.onMouseEnter(() => {
  ...
  nodeDraw.getActionButtonDraw().onClick(() => {
    if (nodeDraw.model.id === "start") {
      ...
      nodeDraw.model.outputs = hasToneOutput ? nodeDraw.model.outputs.filter((port) => port.id !== "tonePreset") : [
        ...nodeDraw.model.outputs,
        { id: "tonePreset", label: "Tone", dataType: "string" }
      ];
      nodeDraw.model.parameters = nodeDraw.model.parameters.map((row) => row.label === "Output Schema" ? {
        ...row,
...
var mouse = new Mouse(canvas, renderManager, scene, select, keyControl, eventManager);
...
mouse.attach();
renderManager.resize(window.innerWidth, window.innerHeight);
var initialBounds = scene.getWorldBounds();
if (initialBounds) {
